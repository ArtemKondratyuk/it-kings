<section class="backdrop js-menu-container menu-container is-hidden" data-modal data-modal2>
    <div class="modal-buy-now">
        <button type="button" class="btn-close-open js-close-menu" data-modal-close>
      <svg width="24" height="24">
        <use href="./img/symbol-defs.svg#icon-close"></use>
      </svg>
    </button>
    <h1 class="header-modal-buy"><span class="acсent">BUY</span> NOW</h1>
    <ul class="chocolate-list">
      <li class="wrap-img-choco">
        <img
          src="./img/chocolate 100.png"
          srcset="
            ./img/chocolate 100.png 1x,
            ./img/chocolate 100.png 2x
          "
          alt="chocolate1"
          width="120"
          class=""
        />
        <button type="submit" class="weight chocolate-selection">100 g</button>
      </li>
      <li class="wrap-img-choco">
        <img
          src="./img/chocolate 200.png"
          srcset="
            ./img/chocolate 200.png 1x,
            ./img/chocolate 200.png 2x
          "
          alt="chocolate2"
          width="120"
          class=""
        />
        <button type="submit" class="weight chocolate-selection">200 g</button>
      </li>
      <li class="wrap-img-choco">
        <img
          src="./img/chocolate 300.png" 
           srcset="
            ./img/chocolate 300.png 1x,
            ./img/chocolate 300.png 2x
          "
          alt="chocolate3"
          width="120"
          class=""
         
        />
        <button type="submit" class="weight chocolate-selection">300 g</button>
      </li>
    </ul>
    <!-- Slider main container -->
    <div class="swiper swiper-wrap-buy-n">
      <!-- Additional required wrapper -->
      <ul class="swiper-wrapper">
        <!-- Slides -->
        <li class="swiper-slide swiper-slide-list">
          <img
            src="./img/chocolate 100.png"
            srcset="
              ./img/chocolate 100.png 1x,
              ./img/chocolate 100.png 2x
            "
            alt="chocolate1"
            width="167"
            class="chocolate"
          />
          <div class="weight">100 g</div>
        </li>
        <li class="swiper-slide swiper-slide-list">
          <img
            src="./img/chocolate 200.png"
            srcset="
              ./img/chocolate 200.png 1x,
              ./img/chocolate 200.png 2x
            "
            alt="chocolate2"
            width="167"
            class="chocolate"
          />
          <div class="weight">200 g</div>
        </li>
        <li class="swiper-slide swiper-slide-list">
          <img
            src="./img/chocolate 300.png"
            alt="chocolate3"
            width="167"
            class="chocolate"
            srcset="
              ./img/chocolate 300.png 1x,
              ./img/chocolate 300.png 2x
            "
          />
          <div class="weight">300 g</div>
        </li>
      </ul>
      <!-- If we need pagination -->
      <div class="swiper-pagination"></div>


      <!-- If we need navigation buttons -->
      <!-- <div class="swiper-button-prev"></div>
            <div class="swiper-button-next"></div> -->

        
            <!-- If we need scrollbar -->
            <div class="swiper-scrollbar"></div>
        </div>
        <form name="order" class="modal-order">
            <ul class="list-form">
                <li>
                    <div>
                        <p class="input-text"><label for="user_name">Personal information</label></p>
                        <ul class="eryrt">
                            <li class="item-form"><input id="user_name" type="text" class="modal-input" placeholder="Name" required ></li>
                            <li class="item-form"><input type="text" class="modal-input" placeholder="Surname" required ></li>
                        </ul>
                    </div>
                </li>
                <li class="item-form">
                    <label for="user_email"><span class="input-text">Email</span>
                        <input id="user_email" type="email" class="modal-input" placeholder="Enter your email" required 
                        pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$">
                    </label>
                </li>
                <li class="item-form">
                    <label for="user_phone"><span class="input-text">Phone number</span>
                        <span class="input-wrap">
                            <input id="user_phone" type="tel" class="modal-input №phone" placeholder="Phone number" required value="+380">
                            <svg width="24" height="18" class="modal-icon-tel">
                                <use href="./img/icons.svg#icon-rame"></use>
                            </svg>
                        </span>
                    </label>
                </li>
                <li class="item-form">
                    <label for="user_card"><span class="input-text">Card number</span>
                       <span class="input-wrap">
                            <input id="user_card" type="number" class="modal-input №card " placeholder="Enter card" required>
                            <svg width="32" height="21" class="modal-icon-card">
                                <use href="./img/icons.svg#icon-mastercard"></use>
                            </svg>
                       </span>
                    </label>
                </li>
                <li class="item-form">
                    <label for="comment">
                        <span class="input-text">Comment</span>
                        <textarea id="comment" class="comment-text" name="comment-user" id=""  rows="4" placeholder="Enter text"></textarea>
                    </label>
                </li>
            </ul>
            <button type="button" class="button-modal-buy" data-modal-close data-modal2-close data-submit-open >Submit</button>
    
          </section>